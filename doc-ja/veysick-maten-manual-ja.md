...
- 【説明】 *ファイル記述子* で指定されたプログラムファイルを読み込み、実行します。 *行番号* は実行開始行を指定します。 *行番号* が省略された場合は、プログラムの最初から実行します。 `ALL` が指定されたら、すべての変数を引き渡します。`ALL`を省略すると `COMMON`文で宣言された変数のみ引き渡されます。`MERGE` オプションを指定すると、プログラムをメモリー上で混合し、混合結果のプログラムを指定された *行番号* から実行します。指定されたプログラムはアスキーセーブしたものでなければなりません。同一の行番号があった場合は、その行は上書きされます。`DELETE` オプションは、`MERGE` オプションが指定された場合のみ有効であり、混合する前に `DELETE` で指定された *範囲* を削除します。行番号の代わりにラベル名を使用できます。
...
タートルの位置は、[LP](#lp)の位置になります。タートルが移動すると[LP](#lp)も移動します。座標はスクリーン座標系で指定します。

|サブコマンド   |条件                 |機能                                                                                   |
|:--------------|:--------------------|:--------------------------------------------------------------------------------------|
|`FDn`          |-32768≦`n`≦32767   |タートルを`n`進めます。タートルの向きは変化しません。                                  |
|`BKn`          |-32768≦`n`≦32767   |タートルを`n`後退します。タートルの向きは変化しません。                                |
|`MVx,y`        |-32768≦`x,y`≦32767 |タートルをスクリーン座標(`x`,`y`)へ移動します。タートルの向きは変化しません。          |
|`SXx`          |-32768≦`x`≦32767   |タートルのX座標をスクリーン座標で設定します。タートルの向きは変化しません。            |
|`SYy`          |-32768≦`y`≦32767   |タートルのY座標をスクリーン座標で設定します。タートルの向きは変化しません。            |
...
- 【説明】 指定したチャンネルの音に、ビブラート(音程を細かく変化させること)やトレモロ(音量を細かく変化させること)をつけることができます。*チャンネル番号*には、`1`～`6`の値を指定します。*WF*には、波形（Wave Form）の種類（`0`～`3`）を指定します。`0`の場合、ノコギリ波で、`1`の場合、矩形波、`2`の場合、三角波、`3`の場合、ランダムの波形になります。*SYNCスイッチ*が `1` の場合、音が発生されるのと同時にLFO波形を発生させ、`0`の場合、音の発生と無関係に LFO 波形を発生させます。*スピード* には `0`～`16383` の値を設定できます。 *PMD*は、Pitch Modulation Depthの略で、ビブラートをかけるときに使います。*PMD*には `-127`～`127` の値を指定でき、絶対値が大きいほど変化の幅が大きくなります。*AMD* は Amplitude Modulation Depth の略で、トレモロをかけるときに使用します。ただし *AMD* を指定できるのは、FM音源の場合のみです。*AMD*には `-127`～`127` の値を指定でき、絶対値が大きいほど変化の幅が大きくなります。*PMS*は Pitch Modulation Sensitivity の略で、`0`～`15`の値を指定でき、ビブラートをかけるときに使います。*PMS*は、*PMD*よりも効果が強いので、通常、*PMS*で大まかな調整をしておいて *PMD*で細かい調整を行います。
...
- 【注意】 C/C++の浮動小数点数は、IEEE 754準拠であり、N88-BASICの浮動小数点数とは異なるため、変換による誤差が発生することがあります。
...
- 【説明】 `DATE$`関数は、内部時計の日付を8文字の文字列で返します。返される文字列の形式は、`年2桁/月2桁/日2桁`です。月は1以上12以下でなければいけません。日は1以上31以下でなければいけません。
- 【注意】 VeySicKでは、システム日時の変更はできません。VeySicKのプロセスはシステム日時と`DATE$`に設定した値の差を一時的に記憶します。VeySicKはサマータイムをサポートしません。
...
※ [グラフィック画面](#graphic_screen)が白黒モードの場合は`M=1`、[8色モード](#color_mode)の場合は`M=3`、[16色モード](#color_mode)の場合は`M=4`となります。

- (配列の添え字の最大値) = (必要なバイト数)`\N+1`

※ 整数型配列の場合`N=2`、単精度型配列の場合`N=4`、倍精度型配列の場合`N=8`となります。
...
- 【注意】 VeySicKの`INP`で現在利用できるのはキーボード入力だけです(VeySicKのみ)。入力ポートの実装は機種に依存します。8801と9801では入力ポートが異なります。
...
- 【書式】 `INPUT$(` *文字数* `[, #`*ファイル番号* `])`
- 【文例】 I$=INPUT$(1)
- 【説明】 *ファイル番号*によって指定されたファイルから、*文字数*分の文字列を読み込み、その文字列を返します。*ファイル番号*が省略された場合は、キーボードから入力が行われますが、[`INPUT`](#input)文とは異なり、入力された文字は表示されません。`INPUT$`関数は、指定された*文字数*の文字列が入力されるまで待ちます。`INPUT$`は、改行文字なども読み込むことができます。*文字数*には`256`以上は指定できません。
...
- 【語源】 In string
- 【書式】 `INSTR([` *開始位置* `, ]` *対象文字列* `,` *検索文字列* `)`
- 【説明】 *対象文字列* の中から *検索文字列* を探し、見つかればその位置を`1`以上のバイト数で返します。*開始位置* には読み始める位置を何番目かで指定します。*開始位置* が省略された場合は、`1`と見なされ、文字列の先頭から探します。*検索文字列*が見つからない場合、*対象文字列*が空文字列(`""`)の場合、*開始位置*が*対象文字列*の長さより大きい場合は、`0`を返します。*検索文字列*に空文字列(`""`)を指定すると、*開始位置*と同じ値を返します。全角文字は2バイト分として計算されます。
...
- 【注意】 セキュリティ上の理由により、管理者権限を持たずにVeySicKフォルダの外側のファイルを消すことはできません。
...
- 【語源】 Kanji in string
- 【書式】 `KINSTR([` *開始位置* `,]` *対象文字列* `,` *検索文字列* `)`
- 【文例】 `PRINT KINSTR("国語算数理科社会", "社会")`　⇒　7
- 【説明】 *対象文字列*の中から*検索文字列*を探し出し、見つかればその位置を、見つからなければ`0`を返します。*開始位置*が指定されれば、*開始位置*番目の文字から探し始めます。*開始位置*が省略された場合は、`1`とみなされ、最初から探し始めます。位置は、バイト数ではなく文字数です。*検索文字列*に空文字列(`""`)を指定すると、`KINSTR`の値は*開始位置*と同じになります。[GRPHモード](#char_mode)では[全角文字](#zenkaku)は表示できませんが、[SJISモード](#char_mode)と同じ動作を行います。
...
### `KNJ$`関数 (9801のみ) {#knj_dollar}

- 【機能】 4字の16進文字列を[全角文字](#zenkaku)に変換します。
- 【語源】 Kanji
- 【書式】 `KNJ$(` *文字列* `)`
- 【文例1】 `PRINT KNJ$("8ABF")`　⇒　漢 (SJISモード)
- 【文例2】 `PRINT KNJ$("1B4B")+KNJ$("4A38")+KNJ$("1B48")`　⇒　文 (JISモード)
- 【説明】 *文字列*に指定したコードを[全角文字](#zenkaku)に変換します。*文字列*が4文字を超える場合、最初の4文字だけが使われます。*文字列*は以下の条件を満たしていなければなりません。
    - *文字列* が4文字以上であること。
    - 最初の4文字が16進数(`0`～`9`、`A`～`F`)の範囲内の文字であること。
    - 最初の4文字がJIS/SJISの全角コードの範囲内あるいはKI/KOコードであること。
- 【注意】 [JISモード](#char_mode)で全角文字を表示するには、全角文字の前後にKI/KOコード(`KNJ$("1B4B")`と`KNJ$("1B48")`)が必要です。
...
- 【書式1】 `KPLOAD` *外字文字コード* `,` *整数型配列名*
- 【書式2】 `KPLOAD` *外字文字コード* `,` *Unicodeコードポイント* `, UNI` (VeySicK拡張・無制限モードのみ)
- 【書式3】 `KPLOAD` *外字文字コード* `,` *シフトJISコード* `, SJIS` (VeySicK拡張・無制限モードのみ)
- 【文例1】 `KPLOAD &H7621,A%` ⇒ 外字 `&H7621`のイメージを `A%`で定義。
- 【文例2】 `KPLOAD &H777E,&H2764,UNI:? CHR$(&H777E)` (VeySicK拡張) ⇒ 外字 `&H777E`をハートマーク (`U+2764`)として定義し表示。
- 【説明】 *外字文字コード*で指定された全角文字の文字イメージを、設定します。*外字文字コード*は、8801モードの場合、`EB9FH`～`EBDDH`(シフトJIS 63文字)、9801とVeySicKの場合、`&H7621`～`&H767E` (JIS 94文字) と `&H7721`～`&H777E` (JIS 94文字) の値が指定できます。*外字文字コード*がこの範囲にない場合は、`Illegal function call`エラーが発生します。*整数型配列名*には、18要素を持つ一次元の整数型配列を指定します。配列の最初の2要素には`16`を代入してください。残りの16要素には、上から順番に16ドットのビットイメージを2バイトの整数として格納してください。*Unicodeコードポイント*と `UNI` を指定するか、*シフトJISコード* と `SJIS` を指定すると、標準フォントの指定文字から文字イメージを取り込みます(VeySicK拡張・無制限モードのみ)。指定した文字は[`PUT@ KANJI`](#putat)の[グラフィック画面](#graphic_screen)や[SJISモード](#char_mode)の[テキスト画面](#text_screen)で使えるようになります。
- 【参照】 [`PUT@ KANJI`](#putat)、サンプルプログラム「`drive1/KPLOAD.BAS`」「`drive3/S34.BAS`」
...
- 【説明】 *ファイル記述子*で指定されたプログラムファイルをメモリー上に読み込みます(ロード)。その前にメモリーにあったプログラムは消され、変数の値も初期化されます。開いているファイルがあった場合はすべて閉じられます。`R`オプションを付けると、ファイルを開いたままプログラムを読み込み、直ちに実行を開始します。プログラムファイルを読み込むと、自動的にそのファイルがあるフォルダへ移動します(VeySicKのみ)。拡張子`.BAS`と`.N88`は省略可能です(VeySicKのみ)。
...
- 【機能】 メモリー上にあるプログラムにプログラムファイルの内容を混合します。
- 【語源】 Merge
- 【書式】 `MERGE` *ファイル記述子*
- 【説明】 メモリー上にあるプログラムに*ファイル記述子*で指定したプログラムをメモリー上で混合します。指定するプログラムは、アスキーセーブしたものでなければなりません。ファイル中にメモリー上と同じ行番号があった場合は、その行は上書きされます。`MERGE`文を実行すると必ずコマンドレベルに戻ります。
...
- 【語源】 Middle
- 【書式】 `MID$(` *文字列* `,` *式1* `, [` *式2* `])`
- 【説明】 *文字列*の*式1*バイト目から、*式2*バイト分の文字列を抜き出して返します。*式2*が省略された場合、*式1*バイト目以降から文字列の最後までを返します。*式1*が*文字列*のバイト数より大きかった場合や、*式2*が`0`の場合は空文字列(`""`)を返します。[全角文字](#zenkaku)はそれぞれ2バイト分となります。*式1*に`0`以下の整数が指定されたり、*式2*に0未満の整数が指定されたら、`Illegal function call`エラーが発生します。`(` *式1* `+` *式2* `- 1)` が *文字列*のバイト数を超えたら、*式1*バイト目以降から文字列の最後までを返します。
...
- 【注意】 セキュリティ上の理由により、管理者権限を持たずにVeySicKフォルダの外側のファイルの名前を変えることはできません。
...
- 【注意】 セキュリティ上の理由により、管理者権限を持たずにVeySicKフォルダの外側のファイルを書き換えることはできません。
...
- 【語源】 Output port
- 【書式】 `OUT` *I/Oアドレス* `,` *数式*
- 【説明】 *I/Oアドレス* は、出力ポートの番号で、*数式*は出力する1バイトのデータです。*I/Oアドレス*、*数式*はともに1バイトの整数値`0`～`255`の整数値で指定します。現在、VeySicKで対応している *I/Oアドレス* は次の通りです：
    - 8801の場合：
        - `OUT &H40, &H00` … BEEP音をOFFにします。
        - `OUT &H40, &H20` … BEEP音をONにします。
        - `OUT &H51, &H00` … テキスト画面を非表示にします。
        - `OUT &H51, &H20` … テキスト画面を表示します。
    - 9801の場合：
        - `OUT &H37, &H06` … BEEP音をOFFにします。
        - `OUT &H37, &H07` … BEEP音をONにします。
        - `OUT &H68, &H08` … 奇数ラスタを表示します。
        - `OUT &H68, &H09` … 奇数ラスタを非表示にします。
        - `OUT &H68, &H0E` … 画面を非表示にします（テキスト画面とグラフィック画面の両方）。
        - `OUT &H68, &H0F` … 画面を表示します（テキスト画面とグラフィック画面の両方）。
- 【注意】 VeySicKではまだ対応していない I/Oアドレスがあります。出力ポートの実装は機種に依存します。8801と9801の出力ポートは異なります。
...
- 【説明】 ワールド座標( *WX* , *WY* )を中心点として、*境界色*で囲まれた領域を塗りつぶします。`STEP`をつけると、[LP](#lp)からの相対座標になります。*領域色*と*境界色*はともに、パレット番号で指定します。*領域色*が省略された場合は、[`COLOR`](#color)で指定された前景色が用いられます。*境界色*が省略された場合は、*領域色*と同じ色が使われます。( *WX* , *WY* )の色が*境界色*と同じ色の場合は、何もしません。[LP](#lp)は、指定した座標に移動します。
...
- 【説明】 *配列変数名*で指定された配列変数の中から *整数*で指定された値を探し、最初に見つかった要素の添え字を返します。*配列変数名*は、1次元の16ビット整数型配列でなければなりません。*開始添え字*は、探し始める添え字を指定します。*開始添え字*が省略された場合は、[`OPTION BASE`](#option_base)で指定した値か、ゼロが使用されます。*増分*は、値を探すときに、添え字をどのくらい増やしていくかを指定します。*増分*の省略時は`1`が使われます。見つからなかった場合は `-1`を返します。*増分*がゼロか負の場合は `Illegal function call`エラーが発生します。
- 【参照】 [`FOR`…`TO`…`STEP`～`NEXT`](#for_to_step_next)、[`OPTION BASE`](#option_base)、サンプルプログラム「`drive1/SEARCH.BAS`」
...
- 【書式1】 `TIME$`
- 【書式2】 `TIME$="HH:MM:SS"`
- 【説明】 コンピューター内部の時計の現在の時刻を取得または設定します。時刻は8文字の文字列で返され、文字列変数に代入したり、[`PRINT`](#print)文で表示したりできます。`TIME$="00:00:00"`と設定することにより、ストップウォッチの代わりにすることもできますが、推奨されません。時は24時間制で0以上24未満でなければいけません。分は0以上60未満です。秒は0以上60以下です。
- 【注意】 VeySicKでは、システム日時の変更はできません。VeySicKのプロセスはシステム日時と`TIME$`に設定した値の差を一時的に記憶します。VeySicKはサマータイムをサポートしません。うるう秒により、秒が`60`になることがありますので、注意してください。
...
- 【説明】 「ビューポート」と呼ばれる、スクリーン上のある長方形領域を設定します。ビューポートの座標は整数値であり、*SX1*、*SY1*、*SX2*、*SY2*が整数でない場合は小数点以下四捨五入されます。( *SX1*, *SY1* )を左上の頂点、( *SX2*, *SY2* )を右下の頂点とするオリジナルスクリーン座標上の長方形を[グラフィック画面](#graphic_screen)の新しい表示領域([ビューポート](#viewport))に設定します。「オリジナルスクリーン座標」とは、ディスプレイ画面のピクセルと１対１に対応する物理的な座標のことです。[`WINDOW`](#window1)文によって指定された[ワールド座標系](#world_coord)の領域に入る図形は、[ビューポート](#viewport)内に表示されるようになります。*領域色*が指定された場合、指定されたパレット番号で[ビューポート](#viewport)内を塗りつぶします。*境界色*が指定された場合、指定されたパレット番号で[ビューポート](#viewport)の外側に線を描きます。[LP](#lp)は、[ビューポート](#viewport)の左上の頂点に設定されます。ビューポートの初期値は、`(0, 0)-(639, 199)`か`(0, 0)-(639, 399)`です。*SX1* `< 0` または *SY1* `< 0` または *SX2* がスクリーンの横幅(`640`)以上か、 *SY2*がスクリーンの縦幅(`200`か`400`)以上のときは `Illegal function call`エラーが発生します。
- 【参照】 [`VIEW`](#view2)関数、[`WINDOW`](#window1)、サンプルプログラム「`drive1/VIEW.BAS`」
...
- 【参照】 [`VIEW`](#view1)文、サンプルプログラム「`drive1/VIEW.BAS`」
...
- 【注意】 VeySicKではキーボード以外の入力ポートを待つことができません。8801と9801では入力ポートが異なります。
