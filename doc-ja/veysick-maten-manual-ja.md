...
- 【説明】 *配列変数名*で指定された配列変数の中から *整数*で指定された値を探し、最初に見つかった要素の添え字を返します。*配列変数名*は、1次元の16ビット整数型配列でなければなりません。*開始添え字*は、探し始める添え字を指定します。*増分*は、値を探すときに、添え字をどのくらい増やしていくかを指定します。*増分*の省略時は`1`が使われます。見つからなかった場合は `-1`を返します。*増分*がゼロか負の場合は `Illegal function call`エラーが発生します。
