...
- フリーエリア: `&HD600`～`&HE5FF` ※実機とは異なる
- テキストVRAM(文字と文字属性)：`&HF3C8H`～`&HFF7F` (サイズ `3000` バイト)×1ページ
- グラフィックVRAM：`&HC000`～`&HFFFF` (サイズ `&H4000` バイト)×3ページ

8801のテキストVRAMとグラフィックVRAMの領域はアドレスが重なっておりますが、同じアドレスで4ページを切り替えてグラフィックVRAMにアクセスできます。
...
9801では、グラフィックVRAMにいわゆる「裏画面」があり、同じアドレスで裏と表の2ページを切り替えて使うことができます。メモリー読み書きの`PEEK`/`POKE`に影響するのは、`SCREEN`文のアクティブページです。
