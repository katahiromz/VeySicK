...
- 【説明】 *外字文字コード*で指定された全角文字の文字イメージを、設定します。*外字文字コード*は、8801モードの場合、`EB9FH`～`EBDDH`(シフトJIS 63文字)、9801モードの場合、`&H7621`～`&H767E` (JIS 94文字) と `&H7721`～`&H777E` (JIS 94文字) の値が指定できます。VeySicKの場合は8801と9801の両方の範囲の値が指定できます。*外字文字コード*がこの範囲にない場合は、`Illegal function call`エラーが発生します。*整数型配列名*には、18要素を持つ一次元の整数型配列を指定します。配列の最初の2要素には`16`を代入してください。残りの16要素には、上から順番に16ドットのビットイメージを2バイトの整数として格納してください。*UTF-16コードポイント*と `UNI` を指定すると、標準フォントのUnicode指定文字から文字イメージを取り込みます(VeySicK拡張)。指定した文字は[`PUT@ KANJI`](#putat)や[SJISモード](#char_mode)の[テキスト画面](#text_screen)で使えるようになります。
