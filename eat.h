...
#define EYEBALL_ALLOCATION_TABLE    5  // Version 5
...
namespace EAT
{
    ...
    template <typename T_SIZE = std::size_t>
    struct ENTRY
    {
        // Types
    ...
    struct MASTER : protected HEAD<T_SIZE>
    {
        ...
        void init(size_t total_size)
        {
            ...
            head_type::m_flags = (uint32_t)size_type_size();
    ...
    template <typename T_SIZE = std::size_t>
    inline void init_master(void *image_ptr, T_SIZE image_size)
    {
    ...
    template <typename T_SIZE = std::size_t>
    inline MASTER<T_SIZE> *master_from_image(void *image_ptr)
    {
        auto master = reinterpret_cast<MASTER<T_SIZE> *>(image_ptr);
        if (!master)
            return NULL;
